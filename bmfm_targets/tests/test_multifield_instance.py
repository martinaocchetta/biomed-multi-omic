...
from bmfm_targets.tokenization.multifield_instance import (
...
def test_can_instantiate_from_str_ints():
    mfi = MultiFieldEncodedInstance(
        metadata={"cell_name": "test"},
        data={"genes": [1, 2, 3], "expressions": [2, 4, 5]},
    )
    assert mfi["genes"] == [1, 2, 3]
    assert mfi["expressions"] == [2, 4, 5]


def test_can_modify_instance():
    mfi = MultiFieldInstance(
        metadata={"cell_name": "test"},
        data={"genes": ["token1"], "expressions": "token2"},
    )
    new_data = {"genes": ["token1", "token2"], "expressions": "token2"}
    mfi.data = new_data


def test_cannot_modify_individual_fields():
    mfi = MultiFieldEncodedInstance(
        metadata={"cell_name": "test"},
        data={"genes": [1, 2, 3], "expressions": [2, 4, 5]},
    )
    with pytest.raises(TypeError):
        mfi.data["genes"] = [1, 2, 3, 4]
...
def test_pad_zero_expressed_genes_random():
    ...
    for mfi in mfis:
        updated_mfi = sample_transforms.pad_zero_expressed_genes(
            mfi, pad_zero_expression_strategy="random", max_length=max_length
        )
        updated_mfis.append(updated_mfi)
